      ...
      <header className="main-header">
        ...
        <div className="header-row1">
          ...
          <div className="header-right">
            ...
            {/* Logout */}
            <button onClick={() => navigate('/logout')} className="header-logout" aria-label="Cerrar sesion">
              <LogOut size={15} />
            </button>
          </div>
        </div>
      ...
      <AnimatePresence>
        ...
        {showRecovery && !isOnline === false && (
          ...
          >
            ...
            Conexion restablecida - sincronizando...
          </motion.div>
        )}
      </AnimatePresence>
...
const HeaderConnectionStatus = () => {
  ...
  const handleManualSync = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.totalProcessed === 0) {
        window.alert("Todo al dia: No hay registros pendientes por subir en este dispositivo.");
        ...
        window.alert(`Sincronizacion Parcial:\n\nSe subieron ${res.successCount} registros con exito.\nQuedan ${res.failedCount} registros resguardados localmente por mala senal o congestion.`);
      } else {
  ...
  if (!isOnline) {
    return (
      <div 
        ...
        title={`Sin conexion. ${pendingCount} registros guardados localmente. Haz clic para intentar sincronizar manualmente.`}
      >
  ...
  return (
    <div 
      ...
      title="Conectado en tiempo real con el servidor. Haz clic para forzar sincronizacion manual."
    >
      <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#22C55E', boxShadow: '0 0 8px #22C55E' }} />
      <span className="connection-status-text">EN LINEA</span>
    </div>
  );
};
