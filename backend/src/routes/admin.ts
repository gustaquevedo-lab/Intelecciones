...
export default function adminRoutes(upload: multer.Multer) {
  ...
  // ── POST /api/admin/system/upload-database (PERMANENTLY DISABLED FOR SAFETY) ─
  router.post('/admin/system/upload-database', (req, res) => {
    return res.status(403).json({
      error: 'ACCESO BLOQUEADO: La sobreescritura de la base de datos de producción ha sido deshabilitada permanentemente para proteger la integridad de las capturas de campo y evitar pérdida de datos.'
    });
  });
