...
const dbPath = path_1.default.join(process.cwd(), 'intellecciones.db');
...
const locals = db.prepare("SELECT DISTINCT local_votacion FROM electors WHERE local_votacion IS NOT NULL AND local_votacion != ''").all();
...
db.transaction(() => {
    for (let i = 0; i < locals.length; i++) {
        ...
        const electores = db.prepare('SELECT ci FROM electors WHERE local_votacion = ? ORDER BY CAST(mesa AS INTEGER) ASC, CAST(orden AS INTEGER) ASC, ci ASC').all(loc);
...
//# sourceMappingURL=recalibrate-local.js.map